...
# 脚本目录：sys.path 加入它，保证能 import 同目录的 three_dog_coral.py
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _SCRIPT_DIR)

# 数据/配置目录：CORAL_DATA_DIR 环境变量（bundle 的 host 插件注入，如 ~/.dsh/coral）
# 优先，缺省回退到脚本所在目录（开发/直跑形态）。coral_config.json 的 paths 是相对路径，
# chdir 到数据目录后按 CWD 解析——数据永远落在配置所在处，绝不写进 npm 包目录。
_DATA_DIR = os.environ.get("CORAL_DATA_DIR") or _SCRIPT_DIR
if _DATA_DIR != _SCRIPT_DIR:
...
def main() -> int:
    try:
        with open(os.path.join(_DATA_DIR, ".mcp_started"), "a", encoding="utf-8") as fh:
