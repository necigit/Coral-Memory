	...
	return class BoardService extends _classSuper {
		...
		async list() {
			...
			const rows = list.map((t) => {
				...
				const updatedAt = typeof th.updated_at === "number" ? Math.round(th.updated_at * 1e3) : now;
				const status = String(th.status ?? "?");
				const lastStep = steps[steps.length - 1];
				const allDone = steps.length > 0 && doneSteps === steps.length;
				const lastDone = lastStep !== void 0 && stepLooksDone(lastStep);
				...
				return {
					...
					updatedAt,
					ageDays: Math.round((now - updatedAt) / 864e5 * 10) / 10,
					isCandidate
				};
			});
			const order = {
			...
			return rows.sort((a, b) => (order[a.status] ?? 9) - (order[b.status] ?? 9) || b.updatedAt - a.updatedAt);
		}
	};
})();
...
//#endregion
export { BoardService, BoardService as default, apply };
