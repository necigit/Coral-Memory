...
import type { BoardStep, BoardThread } from './types.ts'
...
function stepLooksDone(step: BoardStep): boolean {
...
function threadsPath(): string {
  ...
  try {
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(home, 'coral_board.json'), 'utf-8')) as { threadsPath?: string }
...
export class BoardService extends TypertRemoteService {
  ...
  @Remote('list')
  async list(): Promise<BoardThread[]> {
    ...
    const rows = list.map((t) => {
      ...
      const steps = Array.isArray(th.steps) ? th.steps as BoardStep[] : []
