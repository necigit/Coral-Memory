...
export function balanceStorePath(file = join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'balance.json')) {
    return file;
}
/** Read the store; returns null when absent or malformed. */
export async function readStore(file = balanceStorePath()) {
    try {
...
export async function writeStore(store, file = balanceStorePath()) {
 ...
 */
export async function resolveApiKey(entry, credentials) {
    ...
    if (typeof entry.credential === 'string' && entry.credential.length > 0) {
        ...
        if (typeof viaEnv === 'string' && viaEnv.length > 0)
            return viaEnv;
 ...
 */
export async function seedDefault(credentials) {
...
export async function loadStore(credentials, file = balanceStorePath()) {
    const existing = await readStore(file);
    if (existing !== null)
        return existing.apis;
    const seeded = await seedDefault(credentials);
    if (seeded.length > 0)
        await writeStore({ apis: seeded }, file);
    return seeded;
}
/** Strip inline keys before anything crosses the Remote boundary. */
export function stripSecrets(entry) {
...
export function parseBalanceInfos(body) {
 ...
 */
export async function queryBalance(entry, key) {
...
//# sourceMappingURL=balance-store.js.map