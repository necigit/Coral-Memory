...
| 玩家问题 | 答案（标注前提与来源） |
|---|---|
| **能提升命中率吗？** | 基准测试下能。跨项目共享池场景冷启动 **0% → 100%** 有结果；相关查询 recall@5 = **1.0**；共享+亲缘度方案 precision@5 = **1.0**（合成语料、hash 嵌入的确定性基准；实测于 `benchmarks/bench_cross_project.py`，真实场景随语料/模型浮动） |
| **能省上下文吗？** | **有条件地能**。前提：应用把"全量会话历史"替换为"Top-5 相关记忆"注入。该前提下实测 200 轮会话原文 ≈ 4900 token（按常用中英 tokenizer 估算，可复现于 `tests/test_200_turns.py` 的语料）→ 每次注入 ≈ 126 token（省 ~97%，为估算值）。若应用本来就只带最近几轮对话，记忆层带来的是**长期记忆与个性化**，不是 token 节省 |
| **会越用越卡吗？** | 不会。热度淘汰自动清理没用的旧记忆；实测 2 万条写入 **83.7s**（4.2ms/条）、检索 **12ms/次**、stats() ≈0ms（本机 8C/16T，hash 嵌入，复现于 `stress/stress_20k.py`） |
| **要重新说一遍吗？** | 不用。重复的偏好自动去重合并（200 轮压测实测去重 18 次），命中过的记忆热度更高、更难被淘汰 |

> ⚠️ **数字边界（不夸张承诺）**：本表所有数字来自仓库内**确定性基准**（合成语料、hash 嵌入、固定 seed），
> 脚本已随仓库发布（`benchmarks/`、`tests/`、`stress/`），可自行复现。
> 数字用途是**证明能力上限**，不是效果承诺。真实项目的结果取决于：语料分布、嵌入模型
> （生产默认 `BAAI/bge-small-zh-v1.5`，比基准用的 hash 嵌入语义更准）、检索权重、以及应用如何注入记忆。
